...
cat <<EOF | zexe givenLoadFlatObject.zen -a myFlatObject.json | save . givenLoadFlatObjectOutput.json
...
cat <<EOF | zexe givenLoadNumber.zen -a myFlatObject.json | save . givenLoadNumberOutput.json
Given I have a 'number' named 'myNumber'
Then print all data
EOF
...
cat <<EOF | zexe givenLoadArrayDebugVerbose.zen -a myFlatObject.json | save . givenDebugOutputVerbose.json
#Given debug
...
cat <<EOF | zexe givenArraysLoad.zen -a givenArraysLoadInput.json | save . givenArraysLoadOutput.json
...
cat <<EOF | zexe givenLoadRepetitveObject.zen -a myNestedRepetitveObject.json | save . givenLoadRepetitveObjectOutput.json
...
cat <<EOF | zexe givenLoadRepetitveObjectDebug.zen -a myNestedRepetitveObject.json | save . givenLoadRepetitveObjectDebugOutput.json
Scenario 'ecdh': let us load some stuff cause it is fun!
Given I am 'Alice'
And I have my  'keypair'
And I have a 'string array' named 'myFiveStrings' inside 'myFirstObject' 
...
cat <<EOF | zexe givenLoadNestedObject.zen  -a myNestedObject.json | save . givenLoadNestedObjectOutput.json
Scenario 'ecdh': let us load some stuff cause it is fun!
# 
...
cat <<EOF | zexe givenLoadArrayDebug.zen -a myFlatObject.json | save . givenDebugOutput.json
Given I have a  'string array' named 'myFiveStrings' 
Given debug 
When I randomize the 'myFiveStrings' array
Then print all data
...
cat <<EOF | zexe givenLoadTripleNestedObject.zen -a myTripleNestedObject.json | save . givenTripleNestedObjectOutput.json
...
cat <<EOF | zexe givenFullList.zen -a myLargeNestedObject.json | save . givenFullList.json
...
cat <<EOF | zexe givenLoadNameFromData.zen -a givenUserData.json | save . givenLoadNameFromDataOutput.json
