...
ZEN.get = function(obj, key, conversion)
   ...
   if t == 'table' then
      res = deepmap(ZEN.CONF.input.encoding.fun, k)
      if conversion then
...
function guess_conversion(obj, definition)
   ...
   if t then
      return ({
         ...
         luatype = objtype,
         raw = obj,
         encoding = ZEN.CONF.output.encoding.name
      })
   end
...
function guess_outcast(cast)
   ...
   elseif ZEN.schemas[uscore(cast, ' ', '_')] then
      return ZEN.CONF.output.encoding.fun
   else
...
function check_codec(value)
   ...
   if ZEN.CODEC[value].zentype == 'schema' then
      ...
      return ZEN.CODEC[value].encoding or ZEN.CONF.output.encoding.name
   end
   return ZEN.CONF.output.encoding.name
end
